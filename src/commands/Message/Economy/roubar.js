...
module.exports = {
  ...
  async run (client, message, args) {
    ...
    const daily = await db.fetch(`rob_${autor.id}`)
    ...
    if (daily !== null && timeout - (Date.now() - daily) > 0) {
      ...
      const sorte = Math.floor(Math.random() * 4) + 1

      if (sorte == 2) {
        const amount = Math.floor(Math.random() * autor_money) + 1
        ...
        const amount = Math.floor(Math.random() * user_money) + 1
        const moneyEmbed = new Discord.MessageEmbed()
