  ...
  async run (client, message, args) {
    ...
    if (!args[0]) {
      ...
      const embed = new MessageEmbed()
        ...
        .addField(`<:Mod:848281446837387295> **Moderação** [${mod.size}]:`, `\`${mod.map(cmd => cmd.name).join(' | ')}\``)
        .addField(`😂 **Diversão** [${fun.size}]:`, `\`${fun.map(cmd => cmd.name).join(' | ')}\``)
        .addField(`<:pontin:852197383974551582>**Miscelânea** [${miscelanea.size}]:`, `\`${miscelanea.map(cmd => cmd.name).join(' | ')}\``)
        .addField(`<:dinheiro:852197651184091216> **Economia** [${economia.size}]:`, `\`${economia.map(cmd => cmd.name).join(' | ')}\``)
