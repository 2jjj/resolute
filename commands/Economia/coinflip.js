...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (isNaN(args[2])) {
      ...
      message.inlineReply("Você não pode apostar RCoins negativos!")
    }
    ...
    if (userbal < args[2]) {
      return message.inlineReply(`💸 **|** ${user} Não tem RCoins suficientes para apostar`)
    }

    if (authorbal < args[2]) {
      return message.inlineReply(`<:x_:856894534071746600> **|** Você não tem RCoins o suficiente para fazer apostas!`)
    }
    ...
    message.inlineReply(reply).then((msg) => {
      ...
      yesCollector.on('collect', () => {
        ...
        if (!args[1] || (args[1].toLowerCase() !== 'cara' && args[1].toLowerCase() !== 'coroa')) {
          ...
          message.inlineReply(`💸 **|** Deu **${array1[rand]}**, você ganhou ${args[2]} RCoins!`);
          db.add(`coins_${message.author.id}`, args[2])
          db.subtract(`coins_${user.id}`, args[2])

        } else if (args[1].toLowerCase() != array1[rand]) {
          message.inlineReply(`💸 **|** Deu **${array1[rand]}**, você perdeu dessa vez! ${user} Você ganhou ${args[2]} RCoins!`);
