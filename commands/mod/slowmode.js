...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        if (args[0] === 'off') {
            ...
            if (currentCooldown === 0) return message.channel.send('<:Time:844591047719125012> Slowmode do canal está desativado.').then(m => m.delete({ timeout: 5000 }));

            embed.setTitle('<:Time:844591047719125012> Slowmode Desativado.')
                .setColor('#00ff00')
        ...
        if (time >= 21600) return message.channel.send('<:Time:844591047719125012> Esse limite de modo lento é muito alto, digite qualquer coisa menor que 6 horas.').then(m => m.delete({ timeout: 5000 }));

        if (currentCooldown === time) return message.channel.send(`<:Time:844591047719125012> | Slowmode já está definido para ${args[0]}`);
